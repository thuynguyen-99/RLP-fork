    ...
    for dataset, gt_root_dir in zip(datasets, gt_root_dirs):
        ...
        for method in methods:
            ...
            for img_file, gt_file in tqdm(zip(image_files, gt_files), 0):
                ...
                input_img, gt_img = (
                    rgb_to_ycbcr(input_img)[:, 0, :, :],
                    rgb_to_ycbcr(gt_img)[:, 0, :, :],
                )

                psnr_val = kornia.metrics.psnr(
                    input_img.unsqueeze(1), gt_img.unsqueeze(1), max_val=1.0
                )
                total_psnr += psnr_val
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate PSNR and SSIM for image results."
    )
    parser.add_argument(
        "--root_dir", type=str, required=True, help="Root directory of results"
    )
    parser.add_argument(
        "--gt_root_dirs",
        type=str,
        nargs="+",
        required=True,
        help="List of ground truth directories, one per dataset",
    )
    parser.add_argument(
        "--datasets", type=str, nargs="+", required=True, help="List of dataset names"
    )
    parser.add_argument(
        "--methods", type=str, nargs="+", required=True, help="List of method names"
    )
    args = parser.parse_args()
    if len(args.gt_root_dirs) != len(args.datasets):
        raise ValueError(
            "The number of gt_root_dirs must match the number of datasets."
        )
